...
<div class="min-h-screen bg-gray-200 py-3">
  <a href="/principal/salaryincrease/show/{{$siid}}">
  ...
  <div class="px-5 mx-auto max-w-7x1">
    <h1 class="mb-12 text-center text-4xl text-black font-bold">Upload File Kebutuhan Pengajuan KGB</h1>
    @if(session('errors'))
    ...
    <div class="w-full mb-8 overflow-hidden rounded-lg shadow-lg">
        ...
        <table class="w-full">
          <tbody class="bg-white">
            <tr class="text-gray-700 text-center">
              ...
              <td class="px-4 py-3 text-ms border">{{$data->refSalaryIncreaseFile->name}}</td>
            </tr>
          </tbody>
        </table>
      </div>
      <div class="flex items-center justify-center mt-10">
        <div class="w-full max-w-md mr-4">
          <form action="{{ route('principalsiuploadfile', array($data->id, $siid))}}" enctype="multipart/form-data" method="POST" class="bg-white shadow-lg rounded px-12 pt-6 pb-8 mb-4 border-2 border-green-500">
            @csrf
