...
<div class="min-h-screen bg-blue-200 py-3">
  ...
  <div class="px-5 mx-auto max-w-7x1">
    ...
    <h1 class="text-center text-2xl text-gray-600 font-bold">Pengajuan Cuti Guru</h1>
  ...
  <h1 class="text-center text-2xl text-gray-600 font-bold">Pengajuan Cuti Kepala Sekolah</h1>
  <div class="w-full mb-8 overflow-hidden rounded-lg shadow-lg">
  <div class="w-full overflow-x-auto">
    <table class="w-full">
      <thead>
      ...
      <tbody class="bg-white">
        ...
        @if($principal->user->role->name == "Kepala Sekolah")
        <tr class="text-gray-700 text-center">
          <td class="px-4 py-3 text-ms border font-semibold">{{$loop->iteration}}</td>
          <td class="px-4 py-3 text-ms border">{{$principal->user->personalData->name}}</td>
          <td class="px-4 py-3 text-ms border">{{$principal->user->personalData->workUnit->name}}</td>
          <td class="px-4 py-3 text-ms border">{{$principal->leaveType->name}}</td>
          <td class="px-4 py-3 text-ms border">{{$principal->leave_duration}} Hari</td>
          <td class="px-4 py-3 text-ms border">{{$principal->start_date->formatLocalized("%d/%m/%Y")}} - {{$principal->end_date->formatLocalized("%d/%m/%Y")}}</td>
          <td class="px-4 py-3 text-ms border">{!! $principal->leave_excuse !!}</td>
          <td class="px-4 py-3 text-ms border">
            <a href="{{ url ('/divisionhead/principal/leavepermission/show', array("$principal->id")) }}" class="text-green-600 hover:text-green-400 mr-2">
              <i class="material-icons-outlined text-base">visibility</i>
            </a>
          </td>
        </tr>
