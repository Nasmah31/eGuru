...
<div class="min-h-screen bg-white py-14">
  <div class="px-5 mx-auto max-w-7x1">
    <h1 class="mb-12 text-center text-3xl text-black font-bold">Form Penambahan Kegiatan Pembelajaran / Bimbingan / Tugas Tertentu</h1>
    <div class="md:flex md:justify-center md:space-x-8 md:px-14">
      <form action="{{ route('teacherptastorepbt', $id)}}" method="POST" class="w-full max-w-lg">
        @csrf
        @if(session('errors'))
        ...
        <div class="flex flex-wrap -mx-3 mb-6">
          <div class="w-full px-3">
            ...
            <select name="reference_activity_credit_score_id" id="reference_activity_credit_score_id" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
              <option>Pilih Kegiatan Tugas Jabatan</option>
              @foreach($datas as $data)
              @if($count < 1)
                  @if($data->activity_item == 'Melaksanakan proses pembelajaran' || $data->activity_item == 'Melaksanakan proses bimbingan')
                  <option value="{{$data->id}}">{{$data->grain_item}}</option>
                  @endif
              @else
                  @if($data->activity_item != 'Melaksanakan proses pembelajaran' || $data->activity_item != 'Melaksanakan proses bimbingan')
                      @if($data->grain_item != 'Menjadi Kepala Sekolah per tahun')
                      <option value="{{$data->id}}">{{$data->grain_item}}</option>
                      @endif
                  @endif
              @endif
        ...
        <div id ="cs" class="flex flex-col -mx-3 mb-6" style="display:none;">
          <div class="w-full px-3">
            ...
            <input name="target_credit_score" id="target_credit_score" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" type="number" step=".001">
          </div>
        </div>
...
<script>
$('#reference_activity_credit_score_id').on('change', function () {
