...
<div class="min-h-screen bg-white py-14">
  <div class="px-5 mx-auto max-w-7x1">
    ...
    <div class="md:flex md:justify-center md:space-x-8 md:px-14">
      <form action="/teacher/newpromotion/store" method="POST" class="w-full max-w-lg" enctype="multipart/form-data">
        @csrf
        @if(session('errors'))
        ...
        <div class="flex flex-wrap -mx-3 mb-6">
          <div class="w-full px-3">
            ...
            <select name="new_assesment_credit_id" id="new_assesment_credit_id" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
              <option>Pilih PAK Terakhir</option>
              @foreach($assesments as $assesment)
              <option value="{{$assesment->id}}">{{$assesment->newPerformanceTarget->assessment_year}}</option>
              @endforeach
            </select>
          </div>
        </div>
