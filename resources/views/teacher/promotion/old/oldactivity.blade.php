...
<div class="min-h-screen bg-white py-14">
  <div class="px-5 mx-auto max-w-7x1">
    ...
    <div class="md:flex md:justify-center md:space-x-8 md:px-14">
      <form action="{{ route('teacherpmstoreold', array("$id"))}}" method="POST" class="w-full max-w-lg" enctype="multipart/form-data">
        @csrf
        @if(session('errors'))
        ...
        <div class="flex flex-wrap -mx-3 mb-6">
          <div class="w-full px-3">
            ...
            <input name="old_credit_score" id="old_credit_score" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" type="number" step=".001" required>
          </div>
        </div>
        <div class="md:flex md:items-center">
          <div class="md:w-1/3">
            ...
            <button onclick="window.location='{{ url ('/teacher/promotion/show', array( $id )) }}'" class="shadow bg-red-600 hover:bg-red-400 focus:shadow-outline focus:outline-none text-white font-bold py-2 px-4 rounded" type="button">
              Cancel
            </button>
          </div>
